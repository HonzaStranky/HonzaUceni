// ============================================
// GAME ENGINE - All 4 game modes, 15 questions max
// ============================================
// Diacritic fix for answers
// 1. Tuhle funkci dej úplně nahoru v game.js (nebo kamkoliv mimo ostatní funkce)
const normalizeText = (text) => {
if (!text) return ""; // Pojistka proti prázdnému textu
return text
    .toString()
    .toLowerCase()
    .trim()
...
const Game = (() => {
    ...
    let state = {
        ...
        subject: '',
        topic: '',
        questions: [],      // shuffled & sliced to 15
        questionIndex: 0,
        score: 0,
        correct: 0,
        wrong: 0,
    ...
    function setup(gameMode, subject, topic, questions) {
        ...
        // Shuffle and limit to MAX_QUESTIONS (except speed mode which recycles)
        let shuffled = shuffle(questions);
        if (gameMode !== 'speed') {
            state.questions = shuffled.slice(0, MAX_QUESTIONS);
        } else {
            // Speed mode: keep all questions (they recycle), but store originals
    ...
    function nextTypingQuestion() {
        ...
        hEl.textContent = item.h || 'Odpověz:';
        state.questionIndex++;
    ...
    function nextChoiceQuestion() {
        ...
        document.getElementById('choice-hint').textContent = item.h || 'Odpověz:';
    ...
    function nextFlashcard() {
        ...
        document.getElementById('flash-hint').textContent = item.h || 'Odpověz:';
    ...
    function startSpeed() {
        ...
        document.getElementById('speed-time-left').textContent = '60s';
    ...
    function startSpeedTimer() {
        ...
        state.speedInterval = setInterval(() => {
            state.speedTimeLeft--;
            document.getElementById('speed-time-left').textContent = formatSpeedTime(state.speedTimeLeft);
            updateSpeedRing();
    ...
    function nextSpeedQuestion() {
        ...
        if (state.questionIndex >= state.questions.length) {
            // Recycle questions for speed mode
    ...
    function startTimer(elementId) {
        ...
        state.timerInterval = setInterval(() => {
            ...
            if (el) el.textContent = formatTime(state.secondsPassed);
        }, 1000);
    }
    ...
    function finishRound() {
        ...
        if (accuracy === 100 && total >= 5) xpGain += 20;
        if (state.maxStreak >= 10) xpGain += 15;
        ...
        document.getElementById('comp-correct').textContent = `${state.correct}/${total}`;
        document.getElementById('comp-accuracy').textContent = accuracy + '%';
        document.getElementById('comp-time').textContent = formatTime(state.secondsPassed);
    ...
    return {
        ...
        flipCard, flashRate,
        finishRound, shuffle, formatTime
    };
})();
